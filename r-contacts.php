...
if(mysqli_num_rows($result) != 0){
    ...
    $result = mysqli_query($dbc,$query);
    if(mysqli_num_rows($result) != 0){
        $fan_lists_count = mysqli_num_rows($result);
    }
    //查询粉丝
    $query = "select af.attention_fan_id af_id,af.attention_id id_1,u1.nickname name_1,p1.icon icon_1,u1.academy academy_1,
        ...
        where af.attention_id = $personal_id or (af.fan_id = $personal_id and af.status = 2) order by af.attention_fan_id desc limit 12";
    $result = mysqli_query($dbc,$query);
    $fan_lists = Array();
    $fan_lists_index = 0;
    if(mysqli_num_rows($result) != 0){
    ...
    //用户关注列表
        if($data['id_2'] == $personal_id &&  $data['status'] == ATTENTION_EACH_OTHER)
            $fan_lists[$fan_lists_index] =array ('af_id'=>$data['af_id'],'id'=>$data['id_1'],'name'=>$data['name_1'],'portrait'=>$data['icon_1'],'academy'=>$data['academy_1'],'status'=>$data['status']);
        else
            $fan_lists[$fan_lists_index] =array ('af_id'=>$data['af_id'],'id'=>$data['id_2'],'name'=>$data['name_2'],'portrait'=>$data['icon_2'],'academy'=>$data['academy_2'],'status'=>$data['status']);
            $fan_lists_index++;
        }
    }
    //查询关注的人的数量
    ...
    $result = mysqli_query($dbc,$query);
    if(mysqli_num_rows($result) != 0){
        $attention_id_count = mysqli_num_rows($result);
    }
...
<body>
    ...
    <div id="container">
        <div class="fixed">
            ...
            <div id="sidebar">
                <a href="contacts.php?id=<?php echo $personal_id;?>"><?php if($is_user){echo '我';}else{echo $name;}?>关注的人(<span><?php echo $attention_id_count;?></span>)</a>
                <div class="tips">
                    <h3>操作提示</h3>
                    <p>
            ...
            foreach($fan_lists as &$al){
                ...
                <div class="user-display" value="<?php echo $al['af_id'];?>">
                    ...
                    <div class="user-info">
                        ...
                        <?php if($is_user){?>
                            <p class='status' value=<?php echo $al['status'];?>><?php echo $al['status']==ATTENTION_EACH_OTHER?'互相关注':''; ?></p>
                        <?php }?>
